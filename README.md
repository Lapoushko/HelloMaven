...
<br/>В ветке origin/main появились новые изменения, которых еще нет в локальном main. Сама ветка (task2-dovgiy) ответвляется от локального main, в которой еще не было изменений из remote main.Пока присутствует разница между удаленным и локальным main, от которого происходит rebase, Git не позволяет запушить изменения
